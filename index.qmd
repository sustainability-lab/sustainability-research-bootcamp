...
[Get Started →](program.qmd){.btn .btn-primary .btn-lg role="button"}
[View Schedule →](days-1-2.qmd){.btn .btn-outline-secondary .btn-lg role="button"}
:::
...
```{mermaid}
flowchart TD
    A[Days 1-2: Python Fundamentals] --> B[Day 3: 40-Question Assessment]
    B --> C[Days 4-6: Machine Learning]
    C --> D[Days 7-9: Research Methods]
    D --> E[Day 10: Integration & Projects]
    
    A --> F[NumPy, Pandas, Matplotlib]
    C --> G[Linear Models, Trees, Neural Networks]
    D --> H[Git, LaTeX, Research Bootcamp]
    E --> I[Final Project Proposal]
```
...
[**Contact Us →**](about.qmd){.btn .btn-outline-secondary .btn-lg}
