...
- **Remote Servers & SSH:**
  - **In-Depth Guide:** [A Beginner's Guide to Remote Servers for ML](https://devansh-lodha.github.io/blog/posts/remote_ssh_servers/remote_ssh_servers.html) (A great resource to reference)
  - **Tool:** [tmux Cheat Sheet](https://tmuxcheatsheet.com/)
  - **VS Code:** [Remote SSH Extension Guide](https://code.visualstudio.com/docs/remote/ssh)
...
1.  **First Connection:** Connect to a remote server using the `ssh` command with the username, IP address, and port provided by your instructor.
2.  **SSH Key Authentication:** Secure your connection and enable passwordless login.
    ...
    ```
3.  **SSH Shortcut:** Create a host alias in your local `~/.ssh/config` file for quick access to the server.

#### **Exercise 7E: Server Management & Persistent Sessions (45 minutes)**
1.  **Server Monitoring:** Log in and practice these essential monitoring commands.
    -   Check GPU usage: `watch nvidia-smi`
    -   Check CPU and memory usage: `htop`
    -   Check your disk usage: `du -sh ~`
2.  **Persistent Sessions with `tmux`:** Run experiments that survive disconnections.
...
2.  Choose one simple dataset (tips, iris, housing) and create a shared GitHub repository.
3.  Each member implements a different ML algorithm on a separate branch, then merges via a pull request.
4.  **Remote Execution:** One team member logs into the remote lab server, clones the final repository, and runs the main training script inside a `tmux` session.
5.  **Documentation:** Create a basic `README.md` file that summarizes the results and includes the command to re-attach to the `tmux` session, proving the experiment is running persistently.
6.  Present the final comparison of approaches and the running remote session.
